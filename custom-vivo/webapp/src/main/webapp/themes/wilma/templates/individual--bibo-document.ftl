...
<#if publishedIn?? && publishedIn.statements?? && publishedIn.statements[0]??>
  <span class="pub_meta"><a href="${urls.base}/search?facet_content-type_ss=publications&facet_journal_ss=${publishedIn.statements[0].object?url}">${publishedIn.statements[0].label}</a>, </span>
</#if>
<#if pubMeta[0].publisher??>
  <span class="pub_meta"><a href="${urls.base}/search?facet_content-type_ss=publications&facet_publisher_ss=${pubMeta[0].publisher?url}">${pubMeta[0].publisherName}</a>, </span>
</#if>
<#if pubMeta[0].issn??>
...
<#list authorAffiliations as authorAffiliation>
  <#if authorAffiliation.affiliation?has_content && authorAffiliation.affiliationName?has_content>
    <#if authorAffiliation.grid?has_content>
      ...
      <#else>
          <li>(${authorAffiliation?index + 1}) <a href="${urls.base}/search?facet_content-type_ss=publications&facet_${authorAffiliation.type!organization}_ss=${authorAffiliation.affiliation?url}">${authorAffiliation.affiliationName}, ${authorAffiliation.grid}<#if authorAffiliation.affiliationAbbreviation??>, ${authorAffiliation.affiliationAbbreviation}</#if></a></li>
      </#if>
    <#else>
      <#if !authorAffiliation.type?has_content>
        <li>(${authorAffiliation?index + 1}) ${authorAffiliation.affiliationName}</li>
      <#else>
          <li>(${authorAffiliation?index + 1}) <a href="${urls.base}/search?facet_content-type_ss=publications&facet_${authorAffiliation.type!organization}_ss=${authorAffiliation.affiliation?url}">${authorAffiliation.affiliationName}</a></li>
      </#if>
    </#if>
  </#if>
</#list>
...
<h2>Links & Metrics</h2>
	...
	<p>
      ...
      <#if pubMeta[0].timesCited??>
        <p>Times Cited: <span><a href="https://app.dimensions.ai/discover/publication?and_subset_publication_citations=${dimensionsid}" target="external">${pubMeta[0].timesCited}</a></span></p>
      </#if>
